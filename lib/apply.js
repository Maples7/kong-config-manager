...
const handleWrongStatusCode = require('../tools/non200_handler');
...
module.exports = function apply(dir, host) {
            ...
            Promise.resolve()
              ...
              .then(itemToDeleteUrl =>
                rp({
                  ...
                  .catch(err => handleWrongStatusCode(err, itemToDeleteUrl, 'DELETE'))
              )
          ),
          // Post
                ...
                : Promise.map(patchItemIds, patchItemId => {
                    ...
                    if (!_.isEqual(localPatchItemObj, remotePatchItemObj)) {
                      ...
                      return rp({
                        ...
                        .catch(err => handleWrongStatusCode(err, patchItemUrl, 'PATCH'));
                    }
                  })
          )
        ]);
  ...
  const firstClassObjs = objs.filter(obj => _.includes(ENUMS.OBJECTS, obj));
